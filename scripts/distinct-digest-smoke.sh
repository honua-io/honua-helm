...
capture_phase() {
  ...
  # Helm --logs prints Pod hooks; collect the completed Job explicitly.
  kubectl logs -n "$namespace" "job/$release-honua-preflight-license-status" | tee -a "$dir/helm-test.log"
  grep -Fq "license status check passed: mode=disabled" "$dir/helm-test.log"
